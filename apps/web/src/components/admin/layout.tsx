  ...
  return (
    <div className="min-h-screen flex bg-background">
      ...
      <aside className={cn(
        ...
        <div className="flex items-center gap-2 p-4 border-b border-white/10">
          ...
          <div>
            ...
            <div className="text-[10px] text-white/60">Admin Portal</div>
          </div>
            ...
            return (
              <Link key={item.href} href={item.href} onClick={() => setSidebarOpen(false)}
                className={cn('flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium transition-colors',
                  isActive ? 'bg-accent text-white shadow-gold' : 'text-white/70 hover:bg-white/10 hover:text-white')}>
      ...
      <div className="flex-1 flex flex-col min-w-0">
        <header className="h-14 border-b border-border flex items-center justify-between px-4 lg:px-6 bg-card/80 backdrop-blur-md sticky top-0 z-30">
          <button className="lg:hidden" onClick={() => setSidebarOpen(true)}><Menu className="h-5 w-5" /></button>
          <div className="flex-1" />
          <div className="flex items-center gap-2">
            <Button variant="ghost" size="icon"><Bell className="h-5 w-5" /></Button>
            <ThemeToggle />
            <div className="h-8 w-8 rounded-full gradient-gold flex items-center justify-center text-white text-xs font-bold">AD</div>
          </div>
        </header>
